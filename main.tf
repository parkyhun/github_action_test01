...
terraform {
 ...
 # terraform 상태관리를 위한 remote 백엔드 설정
    backend "s3" {
    bucket = "tfstate-bucket-c180bcbe"      #생성한 s3 의 버킷 이름
    key = "test01/terraform.tfstate"       # s3 버킷 안에서 상태파일이 저장될 경로와 파일명,lecture/ 하위에 만들어 지도록
    region = "ap-northeast-2"               # 서울 리전
    dynamodb_table = "terraform-lock-test01"       # 미리 준비된 dynamodb 테이블
